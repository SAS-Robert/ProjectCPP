...
std::ofstream mMyfile2("data.txt");
...
void handleSendLiveDataReceived(Smpt_device* const device, const Smpt_ack& ack, Smpt_device* const device_stim)
{
    ...
    temp = m_f.filter(values[1]);
    temp = m_f1.filter(temp);
    temp = m_f2.filter(temp);
    temp = sqrt(temp * temp);
    result = m_f3.filter(temp);
    ...
    if (mMyfile2.is_open())
    {
        //value[1] : raw emg 1 measurement
        //result : filtered data
        mMyfile2 << values[1] << ", " << result << "\n";
    }
  /*
    if (values[1] >= 0.02)
    {
