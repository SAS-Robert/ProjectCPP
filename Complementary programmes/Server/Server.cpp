...
// Modified:
	...
	{
		//* ORIGINAL CODE
		if(display_msg){printf("Waiting for status request...");}
		fflush(stdout);
		...
		{
			if(display_msg){printf("recvfrom() failed with error code : %d", WSAGetLastError());}
			//exit(EXIT_FAILURE);
		}

		//print details of the client/peer and the data received
		//printf("Received packet from %s:%d\n", inet_ntoa(si_other.sin_addr), ntohs(si_other.sin_port));
		if(display_msg){printf("UDP Received data: %s\n", buf);}

		//-------------------------------------------------
		//memset(buf, '\0', BUFLEN);
		strcpy(buf, message);
		if(display_msg){printf("UDP Sending: %s\n", buf);}
		if (sendto(s, buf, BUFLEN, 0, (struct sockaddr*) & si_other, slen) == SOCKET_ERROR)
		{
			if(display_msg){printf("sendto() failed with error code : %d", WSAGetLastError());}
			//exit(EXIT_FAILURE);
		}
    	memset(buf, '\0', BUFLEN);
		Sleep(10);
